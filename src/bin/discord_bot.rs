...
impl Config {
    fn from_env() -> Result<Self> {
        Ok(Self {
            discord_token: parse_env_required("DISCORD_TOKEN")?,
            guild_id: parse_env_u64("GUILD_ID")?,
            application_id: parse_env_u64("APPLICATION_ID")?,
...
fn parse_env_u64(name: &str) -> Result<u64> {
    let raw = parse_env_required(name)?;
    raw.parse::<u64>()
        .with_context(|| format!("{name} must be a valid u64 integer"))
}
...
impl EmbeddingsIndex {
    fn load(path: &str) -> Result<Self> {
        ...
        for line in reader.lines() {
            ...
            let row: RagChunkRow =
                serde_json::from_str(line).context("parse embeddings row")?;
...
impl EventHandler for Handler {
    async fn ready(&self, ctx: DiscordContext, ready: Ready) {
        ...
        let guild_id = GuildId::new(self.state.config.guild_id);
        if let Err(err) = guild_id
            .set_commands(&ctx.http, vec![build_ask_command(), build_ask_paper_command()])
            .await
        {
...
fn build_ask_command() -> CreateCommand {
    CreateCommand::new("ask")
        .description("Ask a question about the GPU research corpus")
        .add_option(
...
fn build_ask_paper_command() -> CreateCommand {
    CreateCommand::new("ask_paper")
        .description("Ask a question about a specific paper")
        .add_option(
            CreateCommandOption::new(
                CommandOptionType::String,
                "paper",
...
async fn handle_ask(
    ...
    command
        ...
        .await
        .context("defer response")?;

    let question = extract_question(ctx, command).await?;
    if question.trim().is_empty() {
        command
            .edit_response(
                &ctx.http,
                EditInteractionResponse::new().content("Please provide a question."),
            )
            .await?;
        return Ok(());
    }
    println!(
        "ask: user={} channel={} question={}",
        command.user.id,
        command.channel_id,
        question
    );
    let query_embedding = match openai_embed(&state.http_client, &state.config, &question).await {
        Ok(embedding) => embedding,
        Err(err) => {
            command
                .edit_response(
                    &ctx.http,
                    EditInteractionResponse::new().content(format!("Error: {err}")),
                )
                .await?;
            return Ok(());
        }
    ...
    println!(
        "ask: user={} channel={} context=dataset chunks={} chars={}",
        command.user.id,
        command.channel_id,
        matches.len(),
        context.len()
...
Context:\n{context}\n\nQuestion: {question}\nAnswer:",
    ...
    let answer = match openai_response_text(&state.http_client, &state.config, &prompt).await {
        Ok(text) => text,
        Err(err) => {
            command
                .edit_response(
...
async fn handle_ask_paper(
    ...
    let paper_query = extract_string_option(ctx, command, "paper").await?;
    let question = extract_string_option(ctx, command, "question").await?;
    let paper_query = paper_query.trim();
    if paper_query.is_empty() {
        command
            .edit_response(
                &ctx.http,
                EditInteractionResponse::new().content("Please provide a paper identifier."),
            )
            .await?;
        return Ok(());
    }
    ...
    println!(
        "ask_paper: user={} channel={} paper_query={} question={}",
        command.user.id,
        command.channel_id,
        paper_query,
        question
...
Run download/ingest to add papers.\nTitles:\n- ",
    ...
    if matches.len() > 1 {
        ...
        options.truncate(8);
        let mut message =
            String::from("Multiple papers matched. Please be more specific.\n- ");
        message.push_str(&options.join("\n- "));
        command
            .edit_response(&ctx.http, EditInteractionResponse::new().content(message))
            .await?;
    ...
    let sections = match build_paper_sections(&chunks, state.config.rag_paper_map_max_chars) {
        Ok(value) => value,
        Err(err) => {
            command
                .edit_response(
    ...
    println!(
        "ask_paper: user={} channel={} source={} chunks={} sections={}",
        command.user.id,
        command.channel_id,
        source,
        chunks.len(),
    ...
    let cached_summary = match load_paper_summary_cache(&state.config, &source, &content_hash) {
        Ok(value) => value,
        Err(err) => {
            eprintln!("Failed to read paper summary cache: {err}");
            None
        }
    };
    let summary = if let Some(summary) = cached_summary {
        println!(
            "ask_paper: user={} channel={} cache=hit source={}",
            command.user.id, command.channel_id, source
        );
        summary
    } else {
        println!(
            "ask_paper: user={} channel={} cache=miss source={}",
            command.user.id, command.channel_id, source
        );
        let section_summaries =
            match summarize_paper_sections(&state.http_client, &state.config, &source, &sections)
                .await
            {
                Ok(value) => value,
                Err(err) => {
        ...
        let summary = match reduce_paper_summary(
            &state.http_client,
            &state.config,
            &source,
            &section_summaries,
...
async fn extract_question(
    ...
    extract_string_option(ctx, command, "question").await
}

async fn extract_string_option(
    ctx: &DiscordContext,
    command: &CommandInteraction,
    name: &str,
) -> Result<String> {
    let Some(opt) = command.data.options.iter().find(|opt| opt.name == name) else {
        respond_ephemeral(ctx, command, &format!("Missing {name} option.")).await?;
        bail!("missing {name} option");
    };

    let value = match opt.value {
        CommandDataOptionValue::String(ref value) => value,
        _ => {
            respond_ephemeral(ctx, command, &format!("Invalid {name} value.")).await?;
...
async fn respond_ephemeral(
    ...
    command
        ...
        .await
        .context("send ephemeral response")?;
    Ok(())
}
...
fn extract_output_text(payload: &str) -> Result<String> {
    let value: serde_json::Value =
        serde_json::from_str(payload).context("parse responses JSON")?;
...
async fn main() -> Result<()> {
    ...
    let http_client = HttpClient::builder()
        .user_agent("research-bot-discord/0.1")
        .build()
    ...
    let mut client = Client::builder(&state.config.discord_token, intents)
        .application_id(ApplicationId::new(state.config.application_id))
        .event_handler(Handler { state })
        .await
        .context("build discord client")?;

    client.start().await.context("start discord client")?;
    Ok(())
}
