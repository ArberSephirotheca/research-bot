...
- Slack bot runs in Socket Mode and responds to `/ask`, `/ask_paper`, `/papers`, and `/sync` slash commands with plain text labels for Paper/Question/Answer/Context. `/ask_paper` opens a modal with paper autocomplete. `/sync` starts the user-level sync service.
